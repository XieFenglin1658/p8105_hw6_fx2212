...
The 95% confidence interval for R² is (0.895, 0.928) and for β₁/β₂ is
(-4738.839, 3804.519).

The distribution of R² shows relatively stable model fit, concentrated
in the 0.935-0.945 range. The β₁/β₂ bootstrap distribution is
characterized by a sharp peak near zero with long tails, particularly on
the negative side. While most estimates are relatively small in
magnitude, the occasional extreme values lead to a wide overall range.
The 95% confidence interval, though narrow compared to the extremes,
still spans both positive and negative values, reflecting substantial
uncertainty in the ratio’s sign and magnitude.
...
321.8, model 1 (using only birth length and gestational age) has an RMSE
of 334.2, and model 2 (using head circumference, length, sex, and all
interactions) has an RMSE of 290.2. The proposed model performs better
