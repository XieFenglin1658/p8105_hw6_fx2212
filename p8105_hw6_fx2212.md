...
The 95% confidence interval for R² is (0.894, 0.928) and for β₁/β₂ is
(-5409.223, 4903.453).
...
characterized by a sharp peak near zero with long tails. While most
estimates are relatively small in magnitude, the occasional extreme
values lead to a wide overall range. The 95% confidence interval, though
narrow compared to the extremes, still spans both positive and negative
values, reflecting substantial uncertainty in the ratio’s sign and
magnitude.
...
322.1, model 1 (using only birth length and gestational age) has an RMSE
of 334.5, and model 2 (using head circumference, length, sex, and all
interactions) has an RMSE of 290.1. The proposed model performs better
