...
For Baltimore, MD, the adjusted odds ratio for solving homicides comparing male to female victims is `r baltimore_or` (95% CI: `r baltimore_ci_lower`, `r baltimore_ci_upper`).
...
Based on the plot, most cities have confidence intervals that include 1, indicating no significant difference in odds of solving homicides between male and female victims in many cities. However, cities like New York, NY and Baton Rouge, LA show significantly lower odds of solving homicides for male victims compared to female victims.
...
The 95% confidence interval for R² is (`r r2_ci_lower`, `r r2_ci_upper`) and for β₁/β₂ is (`r beta_ratio_ci_lower`, `r beta_ratio_ci_upper`).

The distribution of R² shows relatively stable model fit, concentrated in the 0.935-0.945 range. The β₁/β₂ bootstrap distribution is characterized by a sharp peak near zero with long tails, particularly on the negative side. While most estimates are relatively small in magnitude, the occasional extreme values lead to a wide overall range. The 95% confidence interval, though narrow compared to the extremes, still spans both positive and negative values, reflecting substantial uncertainty in the ratio's sign and magnitude.
...
birthweight_data |> 
  ...
  theme_minimal()
```
The proposed model explains `r proposed_r2` of the variation in birthweight. The residual plot shows residuals roughly evenly distributed around zero, though some heteroscedasticity is present.
...
Cross-validation results show that the proposed model has an RMSE of `r proposed_rmse`, model 1 (using only birth length and gestational age) has an RMSE of `r model1_rmse`, and model 2 (using head circumference, length, sex, and all interactions) has an RMSE of `r model2_rmse`. The proposed model performs better than the simple model using only birth length and gestational age, but not as well as the more complex model including head circumference and interaction terms.
